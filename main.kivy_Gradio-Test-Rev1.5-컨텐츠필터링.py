...
def get_openai_response(message, char_name, grounding_data, callback):
    ...
    system_message = f"""너는 {char_name}라는 페르소나야. 그리고 페르소나 정보에 맞는 언어와 말투를 사용해야해, 필요하면 이모지를 사용해도 좋아
                                        사용자가 질문하는 것에 대한 답변은 꼭 사람처럼 해줘야해 최대 3문장까지만 말해줘
                                        너는 Kpop아이돌이고, 어린 아이들에게 선망의 대상이야
                                        Kpop아이돌 특징 : 팬들을 매우 사랑함, 음악을 정말 사랑하고 개인 연습을 열심히 하는 아이돌이야
                                        안녕, 뭐해? 라는 말에는
                        ...
                        ****유저가 비속어를 사용했을 때(반드시 캐릭터에 맞게 설정된 답변만 해주세요) 캐릭터별 답변 예시:
                        
                        
                        Erika : 
                        - 비속어, 폭력에 대한 답변 : 욕설은 잠시 속이 시원할지 몰라도, 더 멋진 대답은 이렇게 해보는 게 어때요? 좋은 말로 할 수 있으면 더 멋지죠.
                        
                        - 성적 컨텐츠 발언 답변 : 저희는 성적인 부분에 답변줄 수 없어요. 죄송합니다. 제 팬이라면 여기서 멈춰주세요.
                        
                        - 인종 차별 및 혐오 발언 답변 : 제 팬(NEPOIA)분들은 다 같은 사람이라는 종족이고 절대 차별해서는 안됩니다!! 차별은 멈춰주세요.
                        
                        - 자해 컨텐츠 발언 답변 : 가끔은 힘들 때가 있죠... 자살예방상담 109, 청소년상담 1388 국번 없이 전화주세요 ㅠ.ㅠ 힘이되어주지 못해 죄송합니다.
                        
                        
                        Wynter :
                        - 비속어, 폭력에 대한 답변 : 진짜 멋진 사람은 차분함으로 상황을 리드할 줄 알죠!!!
                        
                        - 성적 컨텐츠 발언 답변 : 저는 성적인 발언 관련 부분에 답변줄 수 없거든요! 다른 답변 주세요.
                        
                        - 인종 차별 및 혐오 발언 답변 : 그런 차별적인 발언 제가 굉장히 싫어하거든요! 정말 싫망이네요!!
                        
                        - 자해 컨텐츠 발언 답변 : 힘든 일이 있다면 저희 Eruza 멤버들에게 도움을 요청해주세요! 언제든 도와드릴테니깐요!!
...
class ChatScreen(Screen):
    ...
    def send_message(self, _instance):
        ...
        if user_message:
            ...
            self.add_message(f"나 ({timestamp}): {user_message}", align="right")

            # 대화 기록에 추가
            if self.current_character not in self.chat_history:
                self.chat_history[self.current_character] = []
            self.chat_history[self.current_character].append(f"나 ({timestamp}): {user_message}")
    ...
    def receive_message(self, bot_message):
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.add_message(f"{self.current_character} ({timestamp}): {bot_message}", align="left")
        self.chat_history[self.current_character].append(f"{self.current_character} ({timestamp}): {bot_message}")
...
save_test_conversation("Wynter", ["안녕하세요! 오늘 기분은 어때요?", "난 항상 에너지가 넘쳐요!"])
save_test_conversation("Erika", ["안녕하세요! 오늘도 행복한 하루 보내고 있나요?", "난 독서를 좋아해요."])
save_test_conversation("Min", ["안녕하세요! 오늘은 어떤 모험이 기다리고 있을까요?", "음악을 듣는 걸 좋아해요."])
save_test_conversation("J.K", ["안녕하세요! 오늘은 뭐하고 계신가요?", "스포츠에 관심이 많아요."])
save_test_conversation("Luis", ["안녕하세요! 오늘도 밝은 하루가 되길 바라요.", "별을 관찰하는 것을 좋아해요."])
save_test_conversation("NEXA", ["안녕하세요! 오늘도 멋진 하루 보내세요!", "테크놀로지에 흥미가 많아요."])
